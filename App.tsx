...
import { formatKoreanNumber, formatCurrency } from './utils/formatters';
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <header className="text-center space-y-2">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            보유하신 아파트의 시세와 대출금, 임차보증금을 입력하시면 LTV 구간별 한도를 즉시 계산해드립니다. (단위: 만원)
          </p>
        </header>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-50 rounded-lg p-4 flex flex-col gap-2 border border-slate-200">
                    <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="font-semibold text-slate-700">{formatCurrency(totalExistingLoan)}</span>
                    </div>
                    {activeDeduction > 0 && (
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-semibold text-amber-600">-{formatCurrency(activeDeduction)}</span>
                        </div>
                    )}
                    <div className="h-px bg-slate-200 my-1"></div>
                    <div className="flex justify-between items-center">
                        <span className="text-slate-800 font-bold">총 차감액</span>
                        <span className="text-lg font-bold text-slate-900">{formatCurrency(totalExistingLoan + activeDeduction)}</span>
                    </div>
                </div>
