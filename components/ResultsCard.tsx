    ...
    <div className={`rounded-xl border p-6 shadow-sm transition-all duration-300 hover:shadow-md ${themeClasses[colorTheme]}`}>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-2xl font-bold tracking-tight">{formatCurrency(totalLimit)}</p>
          <p className="text-xs opacity-60 font-mono mt-1">{formatKoreanNumber(totalLimit)}</p>
        </div>
        ...
        <div>
          ...
          <p className={`text-3xl font-extrabold tracking-tight ${availableAmount > 0 ? '' : 'opacity-50'}`}>
             {availableAmount > 0 ? formatCurrency(availableAmount) : '0만원 (한도초과)'}
          </p>
          {availableAmount > 0 && (
            <p className="text-xs opacity-60 font-mono mt-1">{formatKoreanNumber(availableAmount)}</p>
          )}
        </div>
