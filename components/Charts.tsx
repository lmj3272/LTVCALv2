      ...
      return (
        <div className="bg-white p-4 border border-slate-200 shadow-lg rounded-lg text-sm">
          ...
          <div className="space-y-1">
             <p className="text-slate-500">기존 대출: <span className="font-medium text-slate-900">{formatCurrency(existing)}</span></p>
             {deduction > 0 && (
                <p className="text-amber-600">{deductionLabel}: <span className="font-medium">{formatCurrency(deduction)}</span></p>
             )}
             <p className="text-indigo-600">추가 가능: <span className="font-bold">{formatCurrency(available)}</span></p>
             <div className="border-t pt-1 mt-1">
               <p className="text-slate-700">총 한도: {formatCurrency(totalUsed + available)}</p>
             </div>
          </div>
        </div>
      );
