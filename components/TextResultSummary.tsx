...
const TextResultSummary: React.FC<TextResultSummaryProps> = ({
  ...
  useEffect(() => {
    ...
    let generatedText = `[사업자대출 계산기 BY 정진에셋 이명진 결과 리포트]\n\n`;
    generatedText += `■ 기본 정보 (단위: 만원)\n`;
    generatedText += `- 아파트 시세: ${propertyValue.toLocaleString()}만원\n`;
    generatedText += `- 점유 형태: ${occupancyType === 'owner' ? '실거주' : '세입자 거주'}\n`;
    generatedText += `- ${deductionLabel}: ${deduction.toLocaleString()}만원\n`;
    generatedText += `- 기존 대출금: ${existingLoan.toLocaleString()}만원\n\n`;
    ...
    results.forEach((res) => {
      ...
      generatedText += `- 총 대출 한도: ${res.totalLimit.toLocaleString()}만원 (${formatKoreanNumber(res.totalLimit)})\n`;
      generatedText += `- 추가 가능 금액: ${res.availableAmount.toLocaleString()}만원 (${formatKoreanNumber(res.availableAmount)})\n\n`;
    });
