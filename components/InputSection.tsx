    ...
    <div className={`flex flex-col gap-2 transition-opacity duration-300 ${disabled ? 'opacity-50' : 'opacity-100'}`}>
      <div className="flex items-center justify-between">
        <label className="text-sm font-semibold text-slate-600 flex items-center gap-2">
          {getIcon()}
          {label} <span className="text-xs text-slate-400 font-normal">(단위: 만원)</span>
        </label>
        {onDelete && !disabled && (
          <button 
            onClick={onDelete}
      ...
      <div className="relative">
        <input
          ...
          className={`w-full px-4 py-3 text-lg font-bold border rounded-lg outline-none transition-all pr-16 
            ${disabled 
              ? 'bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed' 
              : 'bg-white border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-slate-800'
            }`}
        />
        <span className={`absolute right-4 top-1/2 -translate-y-1/2 font-medium ${disabled ? 'text-slate-300' : 'text-slate-400'}`}>만원</span>
      </div>
